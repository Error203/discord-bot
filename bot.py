...
class DiscordClient:
	...
	def start_bot(self, token):
		...
		async def on_message(message):
			try:
				self.log.debug(f"{message.author.id}({message.author.name}) said$ {message.content}")
				...
				with open("to_say", "r") as to_say:
					text = to_say.read()

				if used_cmd.startswith(command_prefix):
					if not self.authorized:
						self.log.info(f"{message.author.id} used command$ {message.content}")
							...
							try:
								if command_arguments[0].isdigit():
									try:
										...
										await message.channel.send(f"Вибач, але аргумент **{command_arguments[0]}** не дозволений.")
								...
								requested_member = message.guild.get_member(requested_user_id)
								print(message.guild.get_member(577946641600872487))

								roles = list()

								if requested_member:

									for role in requested_member.roles:
										roles.append(role.name)

										if len(roles) == 1 or len(roles) < 1:
											roles.append("Ніхто(без нашивки)")
					...
					if self.authorized:
						if used_cmd == command_prefix + "runtest":
							self.log.info("running test")
							for i in prefixed_allowed_commands:
								pass
					...
					if used_cmd not in prefixed_allowed_commands:
						if not self.authorized:
							...
							self.log.info(f"{message.author.id} used command# {used_cmd}")
